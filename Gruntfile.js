...
module.exports = function(grunt) {
	...
	var js_sections =  ["head.min.js", "inline.min.js", "polyfill.min.js"]; // grunt --js-sections "a,b,c"
			...
			js_assets: {
				options: {
					...
					process: function(src, filepath) {
						...
						return "/* " + filepath.replace(/^.*?\/bendbabylove.github.io\//, "/") + " */\n" + 
							min.
								replace(/^\s*\/\*[\s\S]*?\*\/\s*/g, "").
								replace(/(^|\n)\/\/[#@] sourceMappingURL=.*?(\n|$)/g, "").
								...
								replace(/[)}][(][^)]*[)]/g, function(str){
									return str
										.replace(/this([\s,.]*)/g, "window$1")
										.replace(/(?:window\.)?jQuery/g, "window.jQuery");
								});
					}
				},
				files: {
